<%@ Page Language="C#" AutoEventWireup="true" CodeFile="300262.aspx.cs" Inherits="_300262" %>
...
<script language="javascript" type="text/javascript">
	var ac_swidth = 0;					// 實際相片寬度
	var ac_sheight = 0;					// 實際相片高度
	var ac_width = 0;					// 目前相片顯示寬度
	var ac_height = 0;					// 目前相片顯示高度
	var tb_width = <%=tb_width%>;		// 相片顯示區寬度
	var tb_height = <%=tb_height%>;		// 相片顯示區高度
	var maxrow = <%=maxrow%>;
	...
	function thumb_reload() {
		this.windowopenertop = window.opener;
		this.windowopenertop.location.reload();
	...
	function show_doc() {
		...
		show_win("3002622.aspx?fl_url=<%=fl_url_encode%>&fl_name=<%=Server.UrlEncode(fl_name)%>&timestamp=" + timestamp, 400, 200);
	}
	
	// 修改檔名 (避免 IE Cache 的作用，後面加上 timestamp 的變數，實際上沒有用到)
	function pic_edit() {
		var now = new Date();
		var timestamp = now.getMinutes() + now.getSeconds() + now.getMilliseconds();
		show_win("3002624.aspx?fl_url=<%=fl_url_encode%>&fl_name=<%=Server.UrlEncode(fl_name)%>&timestamp=" + timestamp, 400, 200);l
	}
</script>
</head>
<body background="../images/background/s21.jpg" onload="chk_imgload()">
	...
	<tr><td align="center" valign="middle">
			...
			<tr><td align="center">
					<img id="img_show" alt="" src="../images/blank.gif" title="<%=fl_name%>" style="border-width:6px; border-style:outset; display:none" />
				</td>
			</tr>
			</table>
		</td>
	...
	<tr><td align="center">
			...
			<img src="../images/line/v-line.gif" alt="" style="border-style:none" />
			<img id="bn_delete" src="../images/button/delete.gif" alt="" title="刪除" class="bnNor" onmouseover="this.className='bnOver'" onmouseout="this.className='bnNor'" onmousedown="this.className='bnDown'" style="border-style:none; margin:0px 5px 0px 5px" onclick="show_win('3002623.aspx?fl_url=<%=fl_url_encode%>&fl_name=<%=Server.UrlEncode(fl_name)%>&rownum=<%=rownum%>', 400, 200)" />
	...
	function init_image() {
		...
		var wCnt = 1.0, hCnt = 1.0, fCnt = 1.0;	
		var imgobj;
		...
		if (imgobj != null) {
			// 圖檔原始尺寸
			ac_swidth = newobj.width;
			ac_sheight = newobj.height;

			wCnt = ac_swidth / (tb_width - 14);		// Table 寬度扣掉 Image 外框寬度+2，計算寬度比例
			hCnt = ac_sheight / (tb_height - 14);	// Table 高度扣掉 Image 外框寬度+2，計算高度比例
			...
			if (fCnt > 1) {
				ac_width = Math.floor(ac_swidth / fCnt);
				ac_height = Math.floor(ac_sheight / fCnt);
			}
			else {
				ac_width = ac_swidth;
	...
	function goPic(row) {
		if (row < 1)
			...
			location.href = "300262.aspx?fl_url=<%=fl_url_encode%>&rownum=" + row + "&maxrow=" + maxrow + "&tbw=" + tbw + "&tbh=" + tbh;
		}
	}
	...
	function show_slide(row) {
		...
		mhref = "30027.aspx?fl_url=<%=fl_url_encode%>&rownum=" + row + "&imgw=" + (window.screen.availWidth - 10).toString();
