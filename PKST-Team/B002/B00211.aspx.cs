...
public partial class _B00211 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		if (!IsPostBack)
		{
			...
			if (Request["sid"] != null && Request["tp_sid"] != null && Request["tq_sort"] != null)
			{
	...
	private void GetData()
	{
				...
				if (mErr == "")
				{
					if (tp_question >= int.Parse(lb_tq_sort.Text))
					{
						...
						if (mErr == "")
						{
							...
							SqlString = "Select ti_sid, ti_desc, ti_correct From Ts_Item Where tp_sid = @tp_sid And tq_sid = @tq_sid Order by ti_sort;";
							...
							switch (lb_tq_type.Text)
							{
								case "0":
									...
									while (Sql_Reader.Read())
									{
										...
										rb_tmp[icnt].Text = Sql_Reader["ti_desc"].ToString().Trim() + "<br>";
								...
								default:
									...
									while (Sql_Reader.Read())
									{
										...
										cb_tmp[icnt].Text = Sql_Reader["ti_desc"].ToString().Trim() + "<br>";
		...
		{
			if (mErr == "End")
				ClientScript.RegisterStartupScript(this.GetType(), "ClientScript", "location.replace(\"B002111.aspx?sid=" + lb_tu_sid.Text + "\");", true);
			else
	...
	protected void lk_ok_Click(object sender, EventArgs e)
	{
		...
		if (mErr == "")
		{
			...
			{
				// 答題結束
				ClientScript.RegisterStartupScript(this.GetType(), "ClientScript", "location.replace(\"B002111.aspx?sid=" + lb_tu_sid.Text + "&tp_sid=" + lb_tp_sid.Text + "\");", true);
			}
			else
			{
				// 續續答下一題
				ClientScript.RegisterStartupScript(this.GetType(), "ClientScript", "location.replace(\"B00211.aspx?sid=" + lb_tu_sid.Text + "&tp_sid=" + lb_tp_sid.Text + "&tq_sort=" + tq_sort.ToString() + "\");", true);
			}
		}
		else
			ClientScript.RegisterStartupScript(this.GetType(), "ClientScript", "alert(\"" + mErr + "\");", true);
