<%@ Page Language="C#" AutoEventWireup="true" CodeFile="2001.aspx.cs" Inherits="_2001" %>
...
<body>
    ...
    <div>
        <center>
            ...
            <table width="98%" border="1" cellspacing="0" cellpadding="4"  bgcolor="#EFEFEF"  class="text9pt"
                ...
                <tr align="center">
                    ...
                    <td>
                        <a href="2001_add.aspx?pageid=<%=lb_pageid.Text %>&fc_name=<%=Server.UrlEncode(tb_fc_name.Text)%>&fc_ext=<%=Server.UrlEncode(tb_fc_ext.Text)%>&fc_desc=<%=Server.UrlEncode(tb_fc_desc.Text)%>"
                            class="abtn">&nbsp;上傳檔案&nbsp;</a>
                    </td>
                </tr>
            </table>
            <asp:GridView ID="gv_Fi_Content" runat="server" AutoGenerateColumns="False" DataKeyNames="fc_sid"
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="下載" ShowHeader="False">
                        <ItemTemplate>
                            <a href="<%# Eval("fl_url") %><%# Eval("fc_name") %>" class="abtn" target="_blank">&nbsp;下載&nbsp;</a>
                        </ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText="刪除" ShowHeader="False">
                        <ItemTemplate>
                            <a href="javascript:mdel(<%# Eval("fc_sid") %>,'<%# Eval("fc_name") %>');" class="abtn"
                                title="使用 javascript 及 iframe 的方式呼叫另一個頁面來刪除資料">&nbsp;刪除&nbsp;</a>
        ...
        <script language="javascript" type="text/javascript">
            function mdel(msid, fcname) {
                if (confirm("確定要刪除「" + fcname + "」?\n(使用javascrip 及 iframe 方式刪除)")) {
                    update.location.replace("2001_del.aspx?sid=" + msid);
                }
            }
        </script>
