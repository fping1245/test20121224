...
public partial class _B0041 : System.Web.UI.Page
{
	...
	private void Build_BarChart()
	{
		...
		using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
		{
			using (SqlCommand Sql_Command = new SqlCommand())
			{
				...
				#region 取得最大值
				SqlString = "Select IsNull(Max(cnt), 0) as umax From (Select Count(*) as cnt From Ts_User";
				SqlString += " Where tp_sid = @tp_sid Group by Floor(tu_score/10)) as uMax;";
				...
				if (mErr == "")
				{
					SqlString = "Select Floor(tu_score/10) * 10 as score, Count(*) as cnt From Ts_User";
					...
					if (Sql_Reader.Read())
					{
						do
						{
							...
							score = int.Parse(Sql_Reader["score"].ToString());
							icnt = int.Parse(Sql_Reader["cnt"].ToString());

							tbl_tmp.Style.Add("width", "100%");

							tr_name_cell.Text = (score).ToString() + " 分；共 " + icnt + " 人。";
