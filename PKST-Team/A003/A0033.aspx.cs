...
public partial class _A0033 : System.Web.UI.Page
{
	...
	protected void lk_save_Click(object sender, EventArgs e)
	{
		...
		if (mErr == "")
		{
			...
			{
				SqlString = "Update Bt_Head Set bh_title = @bh_title, is_check = @is_check, bh_topic = @bh_topic";
				SqlString += ", init_time = getdate() Where bh_sid = @bh_sid";
			...
			ClientScript.RegisterStartupScript(this.GetType(), "ClientScript", "alert(\"「票選主題」修改完成!\\n請繼續處理「票選項目」!\\n\");parent.show_win(\"A0035.aspx?sid=" + lb_bh_sid.Text + "\", -1, -1);", true);
		}
		else
	...
	private bool GetData()
	{
		...
		using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
		{
			SqlString = "Select Top 1 bh_title, is_check, bh_topic From Bt_Head Where bh_sid = @bh_sid";
			using (SqlCommand Sql_Command = new SqlCommand(SqlString, Sql_Conn))
			{
				Sql_Conn.Open();
				Sql_Command.Parameters.AddWithValue("bh_sid", lb_bh_sid.Text);
				...
				using (SqlDataReader Sql_Reader = Sql_Command.ExecuteReader())
				{
					if (Sql_Reader.Read())
					{
						tb_bh_title.Text = Sql_Reader["bh_title"].ToString().Trim();
						tb_bh_topic.Text = Sql_Reader["bh_topic"].ToString().Trim();
