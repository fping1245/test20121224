...
public partial class _A0037 : System.Web.UI.Page
{
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!IsPostBack)
		{
			...
			if (Request["sid"] != null)
			{
				if (int.TryParse(Request["sid"], out bh_sid))
				{
					...
					ods_Bt_Ballot.SelectParameters["bh_sid"].DefaultValue = bh_sid.ToString();
		...
		{
			#region 檢查頁數是否超過
			ods_Bt_Ballot.DataBind();
			gv_Bt_Ballot.DataBind();
			if (gv_Bt_Ballot.PageCount < gv_Bt_Ballot.PageIndex)
				gv_Bt_Ballot.PageIndex = gv_Bt_Ballot.PageCount;

			lb_pageid.Text = gv_Bt_Ballot.PageIndex.ToString();
			#endregion
		}
		else
	...
	// 換頁
	protected void gv_Bt_Ballot_PageIndexChanged(object sender, GridViewPageEventArgs e)
	{
		lb_pageid.Text = gv_Bt_Ballot.PageIndex.ToString();
	}
}
