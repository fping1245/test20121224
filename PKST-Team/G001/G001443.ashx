<%@ WebHandler Language="C#" Class="_G001443" %>
//---------------------------------------------------------------------------- 
//程式功能	資料庫規格管理 > 資料表清單 > 欄位清單 > 刪除欄位
//---------------------------------------------------------------------------- 
using System;
...
public class _G001443 : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context)
	{
		...
		if (context.Request["ds_sid"] != null && context.Request["dt_sid"] != null && context.Request["dr_sid"] != null)
		{
			if (int.TryParse(context.Request["ds_sid"], out ds_sid) && int.TryParse(context.Request["dt_sid"], out dt_sid)
				&& int.TryParse(context.Request["dr_sid"], out dr_sid))
			{
				using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
				{
					using (SqlCommand Sql_Command = new SqlCommand())
					{
						...
						if (mErr == "")
						{
							SqlString = "Delete From Db_Record Where dr_sid = @dr_sid And ds_sid = @ds_sid and dt_sid = @dt_sid;";
							...
							Sql_Command.Parameters.AddWithValue("dr_sid", dr_sid);
