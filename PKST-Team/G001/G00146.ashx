<%@ WebHandler Language="C#" Class="_G00146" %>
//---------------------------------------------------------------------------- 
//程式功能	資料庫規格管理 > 資料表 > 產生單一資料表
//---------------------------------------------------------------------------- 
using System;
...
public class _G00146 : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest (HttpContext context)
	{
		string mErr = "", SqlString = "", ds_code = "", ds_name = "";
			...
			if (int.TryParse(context.Request["ds_sid"], out ds_sid))
			{
				using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
				{
					using (SqlCommand Sql_Command = new SqlCommand())
					{
						...
						#region 取得系統名稱、代號
						SqlString = "Select Top 1 ds_code, ds_name From Db_Sys Where ds_sid = @ds_sid";
						...
						if (mErr == "")
						{
							...
							// 檔名要先編碼，中文檔名才不會有問題
							context.Response.AddHeader("Content-Disposition", "inline;filename=" + context.Server.UrlEncode(ds_code.Replace(" ", "") + "全部表格.doc"));
							...
							#region 取得全部表格說明資料
							SqlString = "Select dt_sid, init_time, dt_modi, dt_name, dt_caption, dt_area, dt_desc, dt_index";
							SqlString += " From Db_Table Where ds_sid = @ds_sid Order by dt_sort";
							...
							if (Sql_Reader.Read())
							{
								do
								{
									#region 換頁處理
									if (dt_sid > 0)
										context.Response.Write("<span lang=3DEN-US style=3D'font-size:2.0pt'><br clear=3Dall style=3D'mso-special-character:line-break;page-break-before:always'><o:p></o:p></span>\n");
									#endregion
									
									dt_sid = int.Parse(Sql_Reader["dt_sid"].ToString());
									...
									#region 建立報表內容
									BuildReport(ds_sid, dt_sid, context);
									#endregion
	...
	private void BuildReport(int ds_sid, int dt_sid, HttpContext context)
	{
		string SqlString = "", dr_name = "";
		SqlDataReader Sql_Reader;
		int dr_sort = -1;
