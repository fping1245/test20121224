...
public partial class _D0023 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
			...
			if (Request["sid"] != null)
			{
				if (int.TryParse(Request["sid"], out ff_sid))
				{
					...
					if (mErr == "")
					{
						...
						ods_Fm_Response.SelectParameters["is_close"].DefaultValue = "";
		...
		if (mErr == "")
		{
			...
			ClientScript.RegisterStartupScript(this.GetType(), "ClientScript", "alert(\"" + mErr + "\");location.replace(\"D002.aspx" + lb_page.Text + "\");", true);
    }
				...
				using (SqlDataReader Sql_Reader = Sql_Command.ExecuteReader())
				{
					...
					{					
						if (mErr == "")
						{
							...
							if (Sql_Reader["is_show"].ToString() == "0" && Sql_Reader["is_close"].ToString() == "0")
							{
								lb_ff_topic.Text = "<font color=\"red\">×關閉×隱藏×</font> " + lb_ff_topic.Text;
								lb_ff_desc.Text = "<font color=\"red\">×關閉×隱藏×</font> " + lb_ff_desc.Text;
							}
							else if (Sql_Reader["is_show"].ToString() == "0" && Sql_Reader["is_close"].ToString() == "1")
							{
								lb_ff_topic.Text = "<font color=\"red\">×× 隱藏 ××</font> " + lb_ff_topic.Text;
								lb_ff_desc.Text = "<font color=\"red\">×× 隱藏 ××</font> " + lb_ff_desc.Text;
							}
							else if (Sql_Reader["is_show"].ToString() == "1" && Sql_Reader["is_close"].ToString() == "0")
							{
								lb_ff_topic.Text = "<font color=\"red\">×× 關閉 ××</font> " + lb_ff_topic.Text;
								lb_ff_desc.Text = "<font color=\"red\">×× 關閉 ××</font> " + lb_ff_desc.Text;
							}
						}
					}
					else
	...
	protected void gv_Fm_Response_RowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		if ((e.Row.RowType == DataControlRowType.DataRow))
		{
				...
				&& DataBinder.Eval(e.Row.DataItem, "is_close").ToString() == "1")
			{
				...
				fr_desc.Text = "<font color=red><b>××× 隱藏  ××× " 
					+ DataBinder.Eval(e.Row.DataItem, "instead").ToString() + "</b></font><br>" + fr_desc.Text;
			}
			#endregion
			...
			if (DataBinder.Eval(e.Row.DataItem, "is_close").ToString() == "0")
			{
				...
				fr_desc.Text = "<p style=\"background-color:#F09AF4; margin:0pt 0pt 0pt 0pt\"><b>××× 關閉 ×××</b><br>" + fr_desc.Text + "</p>";
			}
			#endregion
		}
	}
