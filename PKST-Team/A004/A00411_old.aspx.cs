...
public partial class _A00411_old : System.Web.UI.Page
{
	...
	private void Build_BarChart()
	{
		...
		lt_stat.Text = "<table border=\"1\" cellpadding=\"6\" cellspacing=\"0\" style=\"width:600pt; background-color:#FFFFE0; margin-bottom:5pt\">";
		using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
		{
				...
				using (SqlDataReader Sql_Reader = Sql_Command.ExecuteReader())
				{
					if (Sql_Reader.Read())
					{
						...
						do
						{
							...
							tbwidth = 590 * bi_total / max_num;
							if (tbwidth < 1)
								tbwidth = 1;

							lt_stat.Text += "<tr><td align=left>‧" + Sql_Reader["bi_desc"].ToString().Trim() + "：" + bi_total.ToString("N0") + " 票 (" + bi_percent.ToString("N2") + "%)<br>" ;
							lt_stat.Text += "<table border=2 cellpadding=0 cellspacing=0 style=\"width:" + tbwidth.ToString() + "pt; background-color:#808000; border-bottom-color:Black; border-left-color:White; border-right-color:Black; border-top-color:White;\">";
