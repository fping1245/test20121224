...
public partial class _2002 : System.Web.UI.Page
{
    ...
    protected void gv_Fi_Content_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            // 填入刪除的提示文字
            LinkButton lk_bn = (LinkButton)e.Row.Cells[7].FindControl("lk_del");
            lk_bn.OnClientClick = "return confirm('確定要刪除「" + e.Row.Cells[1].Text + "」?\\n(使用 .Net 方式來刪除資料)');";
        }
    }
    ...
    // 刪除資料
    protected void lk_del_Click(object sender, EventArgs e)
    {
        del_file((LinkButton)sender);
    }

    // 刪除資料
    private void del_file(LinkButton sender)
    {
        ...
        if (int.TryParse(s_sid.Cells[0].Text, out fc_sid))
        {
            #region 刪除資料庫紀錄
