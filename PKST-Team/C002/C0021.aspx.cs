...
public partial class _C0021 : System.Web.UI.Page
{
	static int mb_sid = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		{
			// 檢查使用者權限不存入登入紀錄
			//Check_Power("C002", false);

			// 檢查傳入參數
			if (Request["sid"] != null)
			{
				if (int.TryParse(Request["sid"], out mb_sid))
				{
					// 取得資料
					if (! GetData())
						mErr = "找不到指定的留言資料!\\n";
	...
	private bool GetData()
	{
		...
		{
			SqlString = "Select Top 1 mb_symbol, mb_name, mb_sex, mb_email, mb_time, mb_ip, mb_desc, is_show, instead, is_close";
			SqlString += " From Ms_Board Where mb_sid = @mb_sid";
			...
			using (SqlCommand Sql_Command = new SqlCommand(SqlString, Sql_Conn))
			{
				...
				using (SqlDataReader Sql_Reader = Sql_Command.ExecuteReader())
				{
					if (Sql_Reader.Read())
					{
						tb_mb_name.Text = Sql_Reader["mb_name"].ToString().Trim();
						tb_mb_email.Text = Sql_Reader["mb_email"].ToString().Trim();
						tb_mb_desc.Text = Sql_Reader["mb_desc"].ToString().Trim().Replace("<br>","\r\n");
						tb_instead.Text = Sql_Reader["instead"].ToString();
						lb_mb_time.Text = DateTime.Parse(Sql_Reader["mb_time"].ToString()).ToString("yyyy/MM/dd HH:mm:ss");
						lb_mb_ip.Text = Sql_Reader["mb_ip"].ToString();

						tmpstr = "rb_mb_symbol" + Sql_Reader["mb_symbol"].ToString().PadLeft(2,'0');
						RadioButton mb_symbol = (RadioButton)Page.FindControl(tmpstr);
						if (mb_symbol != null)
							mb_symbol.Checked = true;

						if (Sql_Reader["mb_sex"].ToString() == "1")
						{
							rb_mb_sex1.Checked = true;
							rb_mb_sex2.Checked = false;
						}
						else
						{
							rb_mb_sex1.Checked = false;
							rb_mb_sex2.Checked = true;
						}
	...
	protected void lk_save_Click(object sender, EventArgs e)
	{
		...
		int mb_sex = 0, mb_symbol = 0, icnt = 0, is_show = 0, is_close = 0;
		...
		if (mErr == "")
		{
			using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
			{
				SqlString = "Update Ms_Board Set mb_symbol = @mb_symbol, mb_name = @mb_name, mb_sex = @mb_sex, mb_email = @mb_email";
				SqlString += ", mb_desc = @mb_desc, is_show = @is_show, instead = @instead, is_close = @is_close";
				SqlString += " Where mb_sid = @mb_sid;";
