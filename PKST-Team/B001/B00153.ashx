<%@ WebHandler Language="C#" Class="_B00153" %>
//---------------------------------------------------------------------------- 
//程式功能	考試題庫管理 > 考試紀錄 > 刪除考生資料及答題紀錄
//---------------------------------------------------------------------------- 
using System;
...
public class _B00153 : IHttpHandler, IRequiresSessionState
{
    public void ProcessRequest (HttpContext context)
	{
		...
		{
			if (int.TryParse(context.Request["sid"], out tu_sid) && int.TryParse(context.Request["tp_sid"], out tp_sid))
			{
				using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
				{
					using (SqlCommand Sql_Command = new SqlCommand())
					{
						...
						#region 刪除 Ts_User、Ts_UQuest、Ts_UAns 的紀錄
						if (mErr == "")
						{
							// 刪除 Ts_UAns 的紀錄
							SqlString = "Delete From Ts_UAns Where tu_sid = @tu_sid And tp_sid = @tp_sid;";

							// 刪除 Ts_UQuest 的紀錄
							SqlString += "Delete From Ts_UQuest Where tu_sid = @tu_sid And tp_sid = @tp_sid;";

							// 刪除 Ts_User 的紀錄
							SqlString += "Delete From Ts_User Where tu_sid = @tu_sid And tp_sid = @tp_sid;";
