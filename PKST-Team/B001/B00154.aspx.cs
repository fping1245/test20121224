...
public partial class _B00154 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
		if (!IsPostBack)
		{
			...
			if (Request["tu_sid"] != null && Request["sid"] != null)
			{
				if (int.TryParse(Request["tu_sid"], out tu_sid) && int.TryParse(Request["sid"], out tp_sid))
				{
					...
					#region 接收上一頁傳來的參數	
					lb_page.Text = "?sid=" + tp_sid.ToString();
					...
					if (Request["pageid1"] != null)
						lb_page.Text += "&pageid1=" + Request["pageid1"].Trim();

					if (Request["tu_name"] != null)
						lb_page.Text += "&tb_name=" + Server.UrlEncode(Request["tu_name"].Trim());

					if (Request["tu_no"] != null)
						lb_page.Text += "&tb_no=" + Server.UrlEncode(Request["tu_no"].Trim());

					if (Request["tu_ip"] != null)
						lb_page.Text += "&tb_ip=" + Server.UrlEncode(Request["tu_ip"].Trim());
					#endregion
				}
				else
					mErr = "參數格式錯誤!\\n";
			...
			if (mErr == "")
			{
				...
				ClientScript.RegisterStartupScript(this.GetType(), "ClientScript", "alert(\"" + mErr + "\");location.replace(\"B001.aspx\");", true);
		}
    }
		...
		using (SqlConnection Sql_Conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppSysConnectionString"].ConnectionString))
		{
			...
			using (SqlCommand Sql_Command = new SqlCommand(SqlString, Sql_Conn))
			{
				...
				using (SqlDataReader Sql_Reader = Sql_Command.ExecuteReader())
				{
					...
					{
						tq_item = "<table width=100% border=0 cellpadding=2 cellspacing=0>\n";
						tq_item += "<tr><td><font color=red>※ 尚未輸入答案項目!</font></td>\n</tr></table>\n";
					}
