...
public class ODS_As_Group_DataReader
{
	private string Sql_ConnString = "";

	public ODS_As_Group_DataReader()
	{
		Initialize();
	}
	...
	public SqlDataReader Select_As_Group(string SortColumn, int startRowIndex, int maximumRows, int mg_sid)
	{
		string SqlString = "";

		SqlString = "Select * From (";
		SqlString = SqlString + "Select ag_sid, ag_name, ag_attrib, ag_desc, init_time";
		SqlString = SqlString + ", Row_Number() Over (Order by ";

		// 排序設定
		if (SortColumn.Trim() == "")
			SqlString = SqlString + "ag_name";
		else
			SqlString = SqlString + SortColumn;
		SqlString = SqlString + ") as rownum  From As_Group";
	...
	public int GetCount_As_Group(string SortColumn, int startRowIndex, int maximumRows, int mg_sid)
	{
		int nRows = 0;
		string SqlString = "";
		...
		// 由資料庫中取得筆數
		SqlString = "Select Count(*) as Cnt From As_Group";
		...
		context.Cache["GetCount_As_Group"] = nRows;

		return (int)context.Cache["GetCount_As_Group"];
	}
}